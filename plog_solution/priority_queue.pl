...
solve_zhed( Unorder_Number_squares, [R-C]) :-
    get_vars_matrix(10, 0, Vars),
    sort(Unorder_Number_squares, R-C, [], Number_squares), !, 
...
sort( [], _, New_list, New_list ).
sort([R-C-N |Rest], GR-C, Current_list, New_list ) :-
    append( Current_list, [ R-C-N ], New_List),
    sort(Rest, GR-C, New_List, New_list).

sort([R-C-N |Rest], R-GC, Current_list, New_list ) :-
    append( Current_list, [R-C-N], New_List),
    sort(Rest, R-GC, New_List, New_list).

sort([R-C-N |Rest], GR-GC,Current_list, New_list ) :-
    append( [R-C-N],  Current_list, New_List),
    sort(Rest, GR-GC, New_List, New_list).


solve(_Vars, [], _Visited_Number_squares).
